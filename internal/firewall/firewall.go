...
func runCommand(name string, args ...string) ([]byte, error) {
	...
	if err != nil {
		...
		return output, fmt.Errorf("%s %s failed: %s", name, strings.Join(args, " "), msg)
	}
	return output, nil
}
...
func isAlreadyExistsError(err error) bool {
	...
	msg := err.Error()
	return strings.Contains(msg, "Chain already exists") ||
...
func (m *Manager) Initialize() error {
	...
	for _, c := range chains {
		// 链不存在时创建；其他错误必须暴露，避免页面显示成功但规则没落地。
		if _, err := m.run("iptables", "-t", c.table, "-N", c.chain); err != nil && !isAlreadyExistsError(err) {
			return err
		}
		...
		switch c.chain {
		case ChainInput:
			if err := m.ensureJumpRule("filter", "INPUT", c.chain); err != nil {
				return err
			}
		case ChainForward:
			if err := m.ensureJumpRule("filter", "FORWARD", c.chain); err != nil {
				return err
			}
		case ChainNAT:
			if err := m.ensureJumpRule("nat", "PREROUTING", c.chain); err != nil {
				return err
			}
		}
	}
...
func (m *Manager) ensureJumpRule(table, parentChain, targetChain string) error {
	// Check if jump rule already exists
	if _, err := m.run("iptables", "-t", table, "-C", parentChain, "-j", targetChain); err != nil {
		// Rule doesn't exist, add it
		if _, addErr := m.run("iptables", "-t", table, "-I", parentChain, "1", "-j", targetChain); addErr != nil {
			return addErr
		}
	}
	return nil
...
func (m *Manager) clearNetworkRules(net *database.Network) error {
	...
	for _, t := range tables {
		if err := m.removeRulesWithComment(t.table, t.chain, comment); err != nil {
			return err
		}
	}
	return nil
...
func (m *Manager) removeRulesWithComment(table, chain, comment string) error {
	for {
		// List rules with line numbers
		output, err := m.run("iptables", "-t", table, "-L", chain, "--line-numbers", "-n", "-v")
		if err != nil {
			return err
		}
		...
		for i := len(lines) - 1; i >= 0; i-- {
			if strings.Contains(lines[i], comment) {
				...
				if len(parts) > 0 {
					lineNum := parts[0]
					if _, err := m.run("iptables", "-t", table, "-D", chain, lineNum); err != nil {
						return err
					}
					found = true
...
func (m *Manager) addBlockExternalRule(net *database.Network) error {
	comment := fmt.Sprintf("fcm-%s-block", net.ID[:8])

	// Allow established connections
	if _, err := m.run("iptables", "-A", ChainInput,
		"-d", net.Subnet,
	...
	if _, err := m.run("iptables", "-A", ChainForward,
		"-d", net.Subnet,
		"-m", "state", "--state", "ESTABLISHED,RELATED",
		"-m", "comment", "--comment", comment,
		"-j", "ACCEPT"); err != nil {
	...
	_, err := m.run("iptables", "-A", ChainForward,
		"-d", net.Subnet,
		"-i", net.OutInterface,
		"-m", "comment", "--comment", comment,
		"-j", "DROP")
...
func (m *Manager) applySourceIPRule(net *database.Network, rule *database.FirewallRule, comment string) error {
	...
	args := []string{"-I", ChainInput, "1",
		...
		"-m", "comment", "--comment", comment,
		"-j", "ACCEPT"}
	if _, err := m.run("iptables", args...); err != nil {
		return err
	}
...
func (m *Manager) applyPortForwardRule(net *database.Network, rule *database.FirewallRule, comment string) error {
	...
	if protoFlag == "all" {
		for _, proto := range []string{"tcp", "udp"} {
			if err := m.applyPortForwardRuleForProtocol(net, rule, comment, proto); err != nil {
				return err
			}
		}
		return nil
	...
	return m.applyPortForwardRuleForProtocol(net, rule, comment, protoFlag)
}

func (m *Manager) applyPortForwardRuleForProtocol(net *database.Network, rule *database.FirewallRule, comment, protoFlag string) error {
	...
	if _, err := m.run("iptables", dnatArgs...); err != nil {
	...
	if _, err := m.run("iptables", fwdArgs...); err != nil {
...
func (m *Manager) setupNAT(net *database.Network) error {
	comment := fmt.Sprintf("fcm-%s-nat", net.ID[:8])

	// Enable IP forwarding
	if _, err := m.run("sysctl", "-w", "net.ipv4.ip_forward=1"); err != nil {
		return err
	}
